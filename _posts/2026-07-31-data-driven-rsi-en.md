---
layout: default
title: "Data May Be the First Part of RSI We Can Think Through Clearly"
...
<header class="post-header">
  ...
  <h1>Data May Be the First Part of RSI<br>We Can Think Through Clearly</h1>
  <p class="post-dek">I keep coming back to a simple question: if recursive self-improvement ever begins, what will its earliest form look like? My current bet is data. A model finds the edges of what it cannot do, turns those failures into training experience, learns from them, and goes looking again.</p>
...
If RSI does begin to happen, I doubt it will look very dramatic at first. A model may not begin by rewriting its own architecture. It may begin with something more ordinary: making the next batch of training data better than the last one.

Algorithms obviously matter, but they are difficult to reason about in advance. A new idea often has to be built before we understand why it works, and a change that looks promising at one scale may disappear at another. If you asked me today where the next important algorithmic change will come from, I would not have a confident answer.
...
- How do we evaluate an open-ended goal such as “be better at mathematics” or “be better at coding” well enough to decide what to train next?
- When a batch of synthetic data looks good, how do we know whether the target model can absorb it, and how much of it should we make?
- How do we turn the synthesis tricks people have accumulated across domains into a repeatable production line for useful trajectories?

Those are the three questions I want to stay with here.
...
That is the data wall as I understand it. It does not mean that the internet has run out of new material. It means that data which is new to the model, useful for the next version, and good enough to train on is not growing as quickly as the appetite created by more models and more compute. New tokens no longer guarantee new capability.

> The scarce resource is not tokens. It is experience that is useful for the model’s next step.

If scaling is to continue, we cannot simply wait for people to write a larger internet. Models will have to help create the next round of experience: turn failures into tasks, turn tool and verifier feedback into process, and turn the useful parts of that process back into training data. That is why I see synthetic data as an important precondition for RSI.
...
The total number of generated examples is not very informative by itself. I care more about the share that passes verification, what remains after deduplication, and whether each additional batch of genuinely different trajectories improves an independent evaluation. The first two numbers determine what the production line really costs. The last one tells us whether it should keep running.

Once a small training run shows a signal, accepted and non-duplicate trajectories can be scaled in stages. At each stage I would keep the starting checkpoint, training setup, and evaluation set as stable as possible, and retain the previous rung as a control. If volume grows several times while unfamiliar tasks no longer improve, or if acceptance collapses, the pipeline is approaching saturation.
...
Whenever the target model, generator, or harness changes, I would return to the cheap rungs and run the ladder again. The previous curve is useful evidence, not a guarantee. This is one reason a scaling law for synthetic data is harder to establish than one for natural data.
...
Many synthetic-data pipelines today are essentially prompt + model + filter. That is enough to produce large collections of questions and answers, but it is too thin for what I have in mind. The valuable part is often not the final answer. It is how the model searched, tried something, observed an error, revised a plan, and noticed that it had gone wrong.
...
My reading of Harness–Evolve is straightforward. Put a model in an environment with tools and feedback, and let it make several attempts. It can follow different paths, fail, backtrack, and revise in response to what happens. From those attempts, keep the complete processes that are correct, representative, and meaningfully different from one another.
...
These priors are not meant to write the solution for the model. They point to where exploration is useful, which feedback is trustworthy, and which outcomes should not fool us. Eventually I would like the model to learn more than a single synthesis recipe. Given a new topic or a new open-ended goal, it should be able to borrow from these experiences, design tasks and environments, and search for the next useful data.
...
I am less interested in whether this qualifies as RSI under a strict definition. If a model can help identify its failures, help produce the next training experience, and become stronger because of that experience, the loop is already worth studying.
...
- How do we know an open-ended goal has been covered well enough without quietly replacing it with another benchmark?
- How should trajectories from different harnesses be deduplicated and mixed without cancelling one another out?
- When should we keep scaling the data, and when should we replace the generator or harness instead?
